...
// 템플릿 기반 엔진 로드 - 동기 import
import { grade1Templates } from '../../data/wordProblems/grade1.js';
import { grade2Templates } from '../../data/wordProblems/grade2.js';
import { grade3Templates } from '../../data/wordProblems/grade3.js';
import { grade4Templates } from '../../data/wordProblems/grade4.js';
import { grade5Templates } from '../../data/wordProblems/grade5.js';
import { grade6Templates } from '../../data/wordProblems/grade6.js';
...
export const generateProblemData = (gradeNum, difficulty = 'mixed') => {
    ...
    if (gradeNum === 1) {
        // 새 템플릿 엔진 사용 (1학년 9개 템플릿)
        if (grade1Templates && grade1Templates.length > 0) {
            // 난이도 필터링
            let filteredTemplates = grade1Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade1Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade1Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade1Templates);
            
            // 템플릿 실행을 위한 context 생성
            const context = {
                rand: {
                ...
                answer: {
                    ...
                    multi: (arr) => Array.isArray(arr) ? arr.join(',') : String(arr)
                },
                text: {
                    names: NAMES,
                    items: ITEMS
            ...
            const problem = selectedTemplate.generate(context);
            q = problem.q;
            ans = problem.ans;
            exp = problem.exp;
            hint = problem.hint;
            ...
            if (type === 'addition-total') {
                ...
                const limit = base + Math.floor(Math.random() * 3) + 2;
        ...
        // 새 템플릿 엔진 사용 (2학년 8개 템플릿)
        if (grade2Templates && grade2Templates.length > 0) {
            // 난이도 필터링
            let filteredTemplates = grade2Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade2Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade2Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade2Templates);
        ...
        // 새 템플릿 엔진 사용 (3학년 60개 템플릿)
        if (grade3Templates && grade3Templates.length > 0) {
            const template = getRandom(grade3Templates);
            
            // 난이도 필터링
            let filteredTemplates = grade3Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade3Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade3Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade3Templates);
            ...
            const context = {
                ...
                math: {
                    gcd: (a, b) => b === 0 ? a : arguments.callee(b, a % b),
                    lcm: (a, b) => (a * b) / arguments.callee(a, b),
                ...
                answer: {
                    ...
                    equals: (a, b) => {
                        const normA = b.toString().replace(/\s/g, '').split(',').sort().join(',');
                        const normB = a.toString().replace(/\s/g, '').split(',').sort().join(',');
                        return normA === normB;
                    }
                },
                text: {
                    name: (names) => getRandom(names),
                    item: (items) => getRandom(items),
                    names: (names) => {
                        const n1 = getRandom(names);
                        const n2 = getRandom(names.filter(n => n !== n1));
                        return [n1, n2];
                    }
                },
                NAMES,
        ...
        // 새 템플릿 엔진 사용 (4학년 6개 템플릿)
        if (grade4Templates && grade4Templates.length > 0) {
            // 난이도 필터링
            let filteredTemplates = grade4Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade4Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade4Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade4Templates);
        ...
        // 새 템플릿 엔진 사용 (5학년 5개 템플릿)
        if (grade5Templates && grade5Templates.length > 0) {
            // 난이도 필터링
            let filteredTemplates = grade5Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade5Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade5Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade5Templates);
        ...
        // 새 템플릿 엔진 사용 (6학년 5개 템플릿)
        if (grade6Templates && grade6Templates.length > 0) {
            // 난이도 필터링
            let filteredTemplates = grade6Templates;
            if (difficulty === 'basic') {
                filteredTemplates = grade6Templates.filter(t => t.difficulty === 'basic');
            } else if (difficulty === 'advanced') {
                filteredTemplates = grade6Templates.filter(t => t.difficulty === 'advanced');
            }
            
            const selectedTemplate = filteredTemplates.length > 0 ? getRandom(filteredTemplates) : getRandom(grade6Templates);
